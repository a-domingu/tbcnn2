...
class Pattern_detection():
    ...
    def pattern_detection(self, path, url=False, github_url =None):
        ...
        message = '<p>' + self.pattern.capitalize() + '</p>'
        if url==False:
            message = message + self.print_predictions(predicts, data_dict)
        else:
            message = message + self.print_predictions_url(predicts, data_dict, github_url)
        
        if message == '<p>' + self.pattern.capitalize() + '</p>':
            message = message + '<p> No file has ' + self.pattern.capitalize() + 's </p>' 
    ...
    def print_predictions(self, predicts, data_dict):
        ...
        for data in data_dict.keys():
            if predicts[i] < 0.5:
                ...
                #message = message + '<p> The file ' + name + 'does not have ' + self.pattern + 's</p>'
            else:
                path = data.split(os.path.sep)
                path.pop(0)
                name = os.path.join(*(path))
    ...
    def print_predictions_url(self, predicts, data_dict, github_url):
        ...
        for data in data_dict.keys():
            if predicts[i] < 0.5:
                ...
                #message = message + '<p> The file ' + '<a target="_blank" href=' + url + '>' + url + '</a>' + ' does not have ' + self.pattern + 's</p>'
            else:
                path = data.split(os.path.sep)
                path.pop(0)
                name = os.path.join(path[0], 'blob/master', *(path[1:]))
                url = github_url + '/' + name.replace("\\", "/")
                message = message + '<p> The file ' + '<a target="_blank" href=' + url + '>' + url + '</a>' + ' has ' + self.pattern + 's</p>'
            i+=1
        return message
...
def validate_from_url(url, pattern):
    ...
    pattern_class = getattr(module, class_name)
    pattern_detector = pattern_class(pattern)
    message = pattern_detector.pattern_detection(path, url=True, github_url = site)
    folder_deleter(path)
    return message
